  ...
  useEffect(() => {
    ...
    const checkAuth = async () => {
      try {
        ...
        if (data.session) {
          console.log("Index - Session exists");
          ...
          console.log("Index - No session found, staying on auth page");
        }
      } catch (error) {
        console.error("Index - Error in checkAuth:", error);
      }
    };
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      if (event === 'SIGNED_IN' && session) {
        console.log("Index - User signed in via listener");
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1 text-center">
          <div className="flex justify-center mb-4">
            <img 
              src="/lovable-uploads/3b12ffe1-8c12-4642-8416-27578565b14c.png" 
              alt="Gen X Shred" 
