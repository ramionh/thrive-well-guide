...
-- We'll keep the oldest record for each name

WITH canonical_body_types AS (
...
SET body_type_id = cbt.canonical_id
FROM canonical_body_types cbt
JOIN body_types bt ON bt.id = gender_body_type_ranges.body_type_id
WHERE bt.name = cbt.name
AND gender_body_type_ranges.body_type_id != cbt.canonical_id;
...
WHERE bt.name = cbt.name
AND user_body_types.body_type_id != cbt.canonical_id;
...
UPDATE goals
SET current_body_type_id = cbt.canonical_id
FROM canonical_body_types cbt
JOIN body_types bt ON bt.id = goals.current_body_type_id
WHERE bt.name = cbt.name
AND goals.current_body_type_id != cbt.canonical_id;
...
UPDATE goals
SET goal_body_type_id = cbt.canonical_id
FROM canonical_body_types cbt
JOIN body_types bt ON bt.id = goals.goal_body_type_id
WHERE bt.name = cbt.name
AND goals.goal_body_type_id != cbt.canonical_id;
...
WHERE id NOT IN (
    SELECT canonical_id FROM canonical_body_types
);
