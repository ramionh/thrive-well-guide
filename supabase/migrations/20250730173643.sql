...
latest_checkin AS (
  SELECT DISTINCT ON (wc.user_id)
    ...
    wc.muscle_mass,
    wc.waist_measurement,
    wc.chest_measurement,
    wc.arm_measurement,
    wc.thigh_measurement,
    wc.progress_photos,
...
habit_plan AS (
  SELECT 
    hrg.user_id,
    json_build_object(
      'goals', json_agg(DISTINCT json_build_object('goal_text', hrg.goal_text, 'is_learning_goal', hrg.is_learning_goal)),
      'unwanted_habits', (
...
SELECT DISTINCT
  ...
  json_build_object(
    ...
    'current_weight', lc.current_weight,
    'body_fat_percentage', lc.body_fat_percentage,
    'muscle_mass', lc.muscle_mass,
    'measurements', json_build_object(
      'waist', lc.waist_measurement,
      'chest', lc.chest_measurement,
      'arm', lc.arm_measurement,
      'thigh', lc.thigh_measurement
    ),
    'progress_photos', lc.progress_photos,
